...
class PostHistoryCell: UICollectionViewCell {
    
    var postHistoryController: PostHistoryController?
...
//        imageView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(handleZoomTap)))
    ...
    func handleZoomTap(tapGesture: UITapGestureRecognizer) {
        ...
        if let imageView = tapGesture.view as? UIImageView {
            ...
            self.postHistoryController?.performZoomingForStartingImageView(startingImageView: imageView)
        }
    ...
    override init(frame: CGRect) {
        ...
        profileImageView.leftAnchor.constraint(equalTo: self.leftAnchor, constant: DEVICE_WIDTH * 0.1).isActive = true
        profileImageView.rightAnchor.constraint(equalTo: self.rightAnchor, constant: -DEVICE_WIDTH * 0.1).isActive = false
